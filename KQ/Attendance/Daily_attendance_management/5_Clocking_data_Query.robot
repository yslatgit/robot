...
Suite Setup       G_Switch_To_Action_Page    Xpath=//*[@id="nav-box"]/div[2]/div[2]/div[5]/a
...
export_date_default
    ...
    Wait Until Page Contains    工号
    G_InputText    liufeng5
    Sleep    3
    G_Click_Button    2    #点击导出
    Sleep    5
    ${info}    Get Data From Excel    %{G_FILEPATH}    打卡信息1    #获取导出数据
    ${result}    DB_data    ${sql[5][0]}
    Log    ${info.pop(0)}
...
import_data
    ...
    Sleep    1
    Upload File    %{G_RESUORCE}\\打卡数据导入.xls
    Sleep    1
    ...
    Wait Until Page Contains    工号
    Sleep    10
    #${info}    Get Data From Excel    %{G_RESUORCE}\\打卡数据导入.xls    导入数据
    ${date_lists}    DB_data    ${sql[24][0]}
    Sleep    2
    Should Be Equal As Strings    3    ${date_lists[0][0]}
    [Teardown]    Clear

*** Keywords ***
Clear
    [Documentation]    清除导入的打卡数据
    DB_Connection
    Execute Sql String    ${sql[21][0]}
